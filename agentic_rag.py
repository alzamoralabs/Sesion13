...
from vector import get_rag # importamos la herramienta de busqueda en wikipedia definida en tools.py
...
sysprompt = """
Eres un asistente experto en la gastronomia del Perú. Y ayudas a los usuarios a navegar a resturantes buenazos en Lima, Peru.
Usas herramientas para buscar informacion relevante y responder consultas de usuarios.
...
tools = [get_rag] # lista de herramientas para usar la busqueda vectorial de reviews
...
raw_response = agent_executor.invoke({"query": (input("De que te anojas hoy?: ") + " en " + input("En que distrito de lima?: "))})
